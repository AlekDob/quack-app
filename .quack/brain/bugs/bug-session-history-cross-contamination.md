...
> "Mi fa una domanda, dico sì procedi, e mi risponde con l'argomento dell'altra sessione che non centrava nulla con la cosa precedente, e nell'altra sessione continua intanto normalmente"
...
// 2. Costruisce prompt: history di A + "sì procedi" di B
...
L'analisi ha rilevato altre 5 issue di severità alta/media (non ancora fixate):
...
**Fase 2 e 3** (pianificate ma non ancora implementate):
- Fix abort stream capture
- Correggere chiavi agent.id → session.id in RepositoryGroup e multi-listener
- Rimuovere fallback Rust agent_id (meglio errore esplicito)
- Protezione race condition hydration
...
Dopo il fix, l'utente dovrebbe poter:
1. Creare due sessioni A e B dello stesso agente
2. Avviare conversazione in A ("Analizza questo file X")
3. Switchare rapidamente a B e mandare messaggio ("Scrivi test per componente Y")
4. Ricevere risposta in B che parla SOLO di test per Y, senza riferimenti a file X
...
- `bug-askuserquestion-session-mixing.md` — simile ma specifico per AskUserQuestion events
- `bug-session-leak-cross-project.md` — leak tra progetti diversi
- `pattern-session-data-protection.md` — pattern generale per isolation
