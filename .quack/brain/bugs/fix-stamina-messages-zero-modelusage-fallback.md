...
**Key insight**: The FREE calculation still correctly subtracts the auto-compact reserve (it's a real constraint on usable tokens), but it's not shown in the percentage anymore — only applied to the FREE remaining tokens. This is correct because:
- Percentage should match actual API context usage (what CLI shows)
- FREE is a conservative estimate: `FREE = (MAX - totalUsage) - AUTO_COMPACT_COST`
- The auto-compact reserve is accounted for in FREE but not inflating the total percentage

**Minor discrepancy remaining**: Remaining Messages shows ~4.9k vs CLI's ~1.5k. This is due to overhead approximation differences in the client-side calculation (we estimate ~38k overhead, actual may vary by a few thousand).
