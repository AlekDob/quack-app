...
- `src/components/RepositoryGroup.tsx`, `SortableAgent` component, lines 598-615
- Specifically: outer wrapper div now receives both dnd-kit bindings AND ref
...
- **dnd-kit can now detect drag gestures** — the listener props allow pointer events to be captured
- **Both drag systems coexist without conflict** — dnd-kit listeners on the wrapper, native drag on the child
- **Sidebar reordering is now fully functional** — drag initiation works as expected
- **No regression** — native HTML5 drag to Kanban board unaffected
...
The DOM structure had:
- Outer wrapper: dnd-kit listeners (line 599) - receives `{...listeners}` which includes `onPointerDown`
- Inner agent row: native HTML5 `draggable="true"` with `onDragStart` (line 614)

When a user clicked the agent row, the native `draggable` attribute's built-in handler consumed the pointer event before dnd-kit's PointerSensor could activate. The two drag systems were incompatible on the same visual element.
...
- `src/components/RepositoryGroup.tsx`, `SortableAgent` component, line 614
- Changed from: `<div draggable="true" onDragStart={...}>`
- Changed to: `<div onDragStart={...}>`

### Impact

- **dnd-kit PointerSensor can now activate** — no native `draggable` to block pointer events
- **Sidebar reordering fully functional** — drag gestures are now properly detected and handled
- **No regression to Kanban drag** — if that system is needed elsewhere, it uses different DOM elements
...
Debug logs showed:
```
[DnD] handleDragEnd called with active.id="123abc" over.id="456def"
[DnD] Looking for active.id in branchAgents: NOT FOUND
[DnD] Looking for over.id in allAgents: NOT FOUND
[DnD] currentOrder contains: ["old-id-1", "old-id-2", "old-id-3"]
```

The IDs from dnd-kit (`active.id`, `over.id`) didn't match the IDs in `branchAgents` or `allAgents` props, and `currentOrder` contained stale IDs from `.quack-agent-order.dat` that no longer existed in the rendered agents.
...
Additionally, `currentOrder` was loaded from `.quack-agent-order.dat` once at component mount and never refreshed, so it could contain IDs of agents that no longer existed.
...
**1. Create the ref at component mount:**
```typescript
const renderedOrderRef = useRef<Map<string, string[]>>(new Map());
```

**2. Update the ref during render (after sorting, before JSX):**
```typescript
// Capture the exact order of IDs being rendered for EACH BRANCH
renderedOrderRef.current.set(orderKey, sortedAgents.map(a => a.id));
```

**3. Use the ref in `handleDragEnd` instead of reconstructing:**
```typescript
const handleDragEnd = (event: DragEndEvent) => {
  ...
  // Now activeIndex and overIndex are guaranteed to match the rendered agents
  if (activeIndex === -1 || overIndex === -1) return; // Safety check
  ...
  // Log for debugging
  console.log(`[DnD] Reordered ${orderKey}: moved ${active.id} from index ${activeIndex} to ${overIndex}`);
};
```

### Why This Works

1. **`renderedOrderRef` is updated during render** — it always reflects the exact agents currently in the DOM
2. **dnd-kit uses the same agent IDs** — they come from the `<SortableContext items={agents.map(a => a.id)}>` prop
3. **No prop delay or staleness** — the ref is synchronous, not dependent on async state updates
4. **Safe fallback** — if IDs aren't found (shouldn't happen), we return early instead of corrupting state

### Key Files & Lines

- `src/components/RepositoryGroup.tsx`:
  - Line 1155: Create `renderedOrderRef`
  - Line 2073: Update `renderedOrderRef.current.set(orderKey, sortedAgents.map(a => a.id))`
  - Lines 1356-1401: `handleDragEnd` reads from `renderedOrderRef` instead of reconstructing props
...
- **Drag & drop now works end-to-end** — drag initiation, drop detection, and persistence all succeed
- **ID mismatches eliminated** — using rendered state instead of stale props
- **Debugging greatly improved** — clear logs show when IDs are missing or reorders succeed
- **Robustness** — handles agents being added/removed between drags without corruption
- **Null-safety** — the ref provides a safe source of truth instead of prop reconstruction
