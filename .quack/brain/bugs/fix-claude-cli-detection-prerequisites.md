...
Prerequisites Check onboarding step was not detecting Claude CLI even when installed and running on the system. Git ✓ and Node.js ✓ were correctly detected, but Claude Code CLI showed ✗ red X.
...
# Fails: ENOENT npm directory doesn't exist

# Result: Prerequisites check reports NOT installed ✗
```
...
# Primary check succeeds → reports installed ✓
# Fallback check never executed (not needed)
```
...
- `.quack/brain/patterns/prerequisites-check-onboarding.md` - Full pattern doc

## Cross-Platform

- ✅ **Windows**: `where claude` → `.local/bin/claude` detected
- ✅ **macOS**: `which claude` → works for all install methods
- ✅ **Linux**: Same as macOS (not tested but should work)
...
- ✅ Rebuild app with fix
- ⏳ Test on Windows (current system)
- 🔲 Test on macOS (verify both installation methods work)
- 🔲 Update Prerequisites Check pattern doc if needed
