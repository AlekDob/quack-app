...
Questa condizione era `true` perché il bootstrap agenti sbloccava la UI prima che i progetti fossero caricati.

## Solution

**Una modifica, 7 righe rimosse**: il bootstrap agenti non controlla più `booting`.
...
loadSavedAgents() → setTerminals([...]) → setPersistedProjects(...)
                ...
                React render: terminals=[...], persistedProjects=Map(...)
                                        ↓
         Condizione riga 10301: `false` (ha già dati) → UI completa ✅
```

## Additional Polish

Allungato il fade-out dello splash per maggiore fluidità:
...
**Non lasciare che bootstrap veloci sbloccino la UI prima che i dati critici siano pronti**. Se hai multiple async operations in parallelo, identifica quale è il **gate keeper** (nel nostro caso: il bootstrap principale che carica progetti/terminali) e fai controllare `booting` solo a quello.

## Related Patterns

- `patterns/quack-architecture-overview.md` — Bootstrap sequence
- `patterns/frontend-architecture.md` — App initialization flow
