import type { DailyBriefingData } from './types';
import './JackWidgets.css';

interface Props {
  data: DailyBriefingData;
}

export default function DailyBriefing({ data }: Props) {
  return (
    <div className="jack-widget">
      <div className="jack-widget-header">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
          <line x1="16" y1="2" x2="16" y2="6" />
          <line x1="8" y1="2" x2="8" y2="6" />
          <line x1="3" y1="10" x2="21" y2="10" />
        </svg>
        <span className="jack-widget-title">Daily Briefing</span>
        <span className="briefing-date">{data.date}</span>
      </div>
      <div className="briefing-sections">
        {data.sections.map((section, i) => (
          <div key={i} className="briefing-section">
            <div className="briefing-section-title">{section.title}</div>
            {section.items.map((item, j) => (
              <div key={j} className="briefing-item">{item}</div>
            ))}
          </div>
        ))}
      </div>
    </div>
  );
}
