...
interface AddTerminalWindowModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (name: string, directory: string, color: string, savedCommand?: SavedCommand) => void;
...
export function AddTerminalWindowModal({
  isOpen,
  onClose,
  onConfirm,
  defaultDirectory,
  savedCommands = [],
}: AddTerminalWindowModalProps) {
  ...
  const handleConfirm = useCallback(() => {
    ...
    onConfirm(trimmedName, trimmedPath, finalColor, selectedCommand);
    onClose();
  }, [name, directory, color, customColor, showColorPicker, selectedCommandId, savedCommands, onConfirm, onClose]);
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <h2 className="text-lg font-semibold text-white/90 mb-4">Add Terminal Window</h2>
