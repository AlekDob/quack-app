...
function isoBox(
  g: G,
  ox: number, oy: number,
  eLen: number, sLen: number, h: number,
  topColor: number, frontColor: number, sideColor: number,
) {
  ...
  g.moveTo(t3x, t3y); g.lineTo(t2x, t2y);
  g.lineTo(t2x, t2y + h); g.lineTo(t3x, t3y + h);
  g.closePath();
  g.fill({ color: frontColor });

  // Side face (east-facing: t1→t2 edge, extends down by h)
  g.moveTo(t1x, t1y); g.lineTo(t2x, t2y);
  g.lineTo(t2x, t2y + h); g.lineTo(t1x, t1y + h);
  g.closePath();
  g.fill({ color: sideColor });

  // Top face (drawn last so it's on top)
  g.moveTo(t0x, t0y); g.lineTo(t1x, t1y);
  g.lineTo(t2x, t2y); g.lineTo(t3x, t3y);
  g.closePath();
 ...
 */
function drawSofa(g: G, ox: number, oy: number, eLen: number, faceSouth: boolean) {
  ...
  if (faceSouth) {
    ...
    isoBox(g, ox, oy - backH, eLen, backD, backH, SOFA_BACK_TOP, SOFA_BACK_FRONT, SOFA_SIDE);
    // Seat cushion in front of backrest
    const [, shiftY] = iso(0, 0, 0, backD);
    isoBox(g, ox + backD * SX, oy - seatH + shiftY, eLen, depth, seatH, SOFA_TOP, SOFA_FRONT, SOFA_SIDE);
    ...
    for (let i = 1; i < cushions; i++) {
      ...
      const [lx2, ly2] = iso(ox + backD * SX, oy - seatH + shiftY, step * i, depth);
      g.moveTo(lx, ly); g.lineTo(lx2, ly2);
      g.stroke({ color: SOFA_FRONT, width: 0.7 });
    }
  } else {
    // Backrest on the south side (facing north) — seat first, then backrest behind
    isoBox(g, ox, oy - seatH, eLen, depth, seatH, SOFA_TOP, SOFA_FRONT, SOFA_SIDE);
    // Backrest behind the seat (further south)
    const [bx, by] = iso(ox, oy, 0, depth);
    isoBox(g, bx, by - backH, eLen, backD, backH, SOFA_BACK_TOP, SOFA_BACK_FRONT, SOFA_SIDE);
    // Cushion lines
    const cushions = 3;
    const step = eLen / cushions;
    for (let i = 1; i < cushions; i++) {
...
function drawTV(g: G, ox: number, oy: number) {
  ...
  isoBox(g, standOx, standOy, standE, depth + 2, standH, 0x333333, 0x2a2a2a, 0x222222);

  // Frame: slightly larger than screen
  isoBox(g, ox, oy - screenH - standH, tw, depth + 1, screenH + 2, TV_FRAME, TV_FRAME, 0x111111);
  ...
  const [s0x, s0y] = iso(ox + 1 * EX, oy - screenH - standH + 1 * EY + 2, 0, depth + 1);
  const [s1x, s1y] = iso(ox + 1 * EX, oy - screenH - standH + 1 * EY + 2, tw - 2, depth + 1);
  g.moveTo(s0x, s0y);
  g.lineTo(s1x, s1y);
...
function drawVendingMachine(g: G, ox: number, oy: number) {
  ...
  // Main body
  isoBox(g, ox, oy - h, eLen, sLen, h, VENDING_TOP, VENDING_FRONT, VENDING_SIDE);
...
function OfficeBreakRoom({ position, layer }: OfficeBreakRoomProps) {
  ...
  const drawFurniture = useCallback((g: G) => {
    ...
    // TV higher up near back wall
    drawTV(g, TILE_W * 0.42, TILE_H * 0.08);

    // Vending machine in the far right corner
    drawVendingMachine(g, TILE_W * 0.74, TILE_H * 0.32);
  }, []);
