...
// Imported instance ids may be runtime names rather than Quack provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Quack writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Quack stores canonical provider/options objects.
