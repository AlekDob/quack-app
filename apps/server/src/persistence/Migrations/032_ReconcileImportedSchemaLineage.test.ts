...
layer("032_ReconcileImportedSchemaLineage", (it) => {
  ...
  // tracker has IDs 17-31 recorded under unrelated Quack names. The 17-31
  ...
  // references env_mode.
  it.effect("heals an imported Quack DB whose tracker skipped 17-31", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      // Bring the schema to where Quack and Quack last agreed.
      yield* runMigrations({ toMigrationInclusive: 16 });

      // Mark IDs 17-31 applied under Quack's old names so the migrator
      // skips Quack's renumbered 17-23. Names are illustrative; only the
      ...
      // Seed a thread row with the Quack-era column set so the data-rewrite
  ...
  it.effect("is a no-op on a fresh Quack install", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
