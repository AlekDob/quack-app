...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for Quack's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured Quack automation creation intents.",
      ...
      "- Quack automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed Quack heartbeat automation should stop.",
