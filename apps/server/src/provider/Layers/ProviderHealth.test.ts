...
const allProvidersDisabledServerSettings = {
  ...
  providers: {
    ...
    astronaut: { ...DEFAULT_SERVER_SETTINGS.providers.astronaut, enabled: false },
  },
  ...
  describe("disabled provider handling", () => {
    it("builds an inert status for disabled providers", () => {
      assert.deepStrictEqual(makeDisabledProviderStatus("kilo", "2026-06-16T12:00:00.000Z"), {
        ...
        authStatus: "unknown",
        checkedAt: "2026-06-16T12:00:00.000Z",
        message: "Provider is disabled in Quack settings.",
      });
    });
    ...
    it("projects disabled settings over cached ready statuses", () => {
      ...
      assert.strictEqual(codex?.message, "Provider is disabled in Quack settings.");
    });
    ...
    it.effect("projects cached ready status when a disabled provider is re-enabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(disabledCodex?.message, "Provider is disabled in Quack settings.");
          ...
          assert.notStrictEqual(currentCodex?.message, "Provider is disabled in Quack settings.");
    ...
    it.effect("does not offer updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        for (const status of statuses) {
          ...
          assert.strictEqual(status.message, "Provider is disabled in Quack settings.");
    ...
    it.effect("rejects one-click updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.reason, "Provider is disabled in Quack settings.");
      }).pipe(Effect.provide(disabledProviderHealthLayer)),
    );
  });
  ...
  describe("checkPiProviderStatus", () => {
    ...
    it.effect("uses configured Pi binary and agent dir without SDK registry reads", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Pi CLI is installed. Quack will use Pi agent dir /tmp/pi-agent.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args, command) => {
  ...
  describe("checkAntigravityProviderStatus", () => {
    it.effect("rejects versions that predate --new-project support", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Antigravity CLI 1.0.11 is too old for Quack. Upgrade to 1.0.12 or newer.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args) => {
