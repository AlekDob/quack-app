...
//          that makes Quack catalog skills usable on every provider.
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.synara/skills`, which Quack registers at session start
    ...
    case "cursor":
      ...
      // only Quack-owned paths need inlining.
