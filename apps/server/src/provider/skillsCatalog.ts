...
//          walking) plus the unified cross-provider skills catalog backing Quack
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Quack base dir (usually `~/.synara`); skills live in `{base}/skills`. */
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Quack. */
...
const SKILL_ORIGIN_ROOTS = {
  ...
  codex: {
    // Keep Quack's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Quack as the
