...
describe("discoverSkillsCatalog", () => {
  it("creates the Quack skills folder on first discovery", async () => {
  ...
  it("aggregates skills from synara and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(synaraBaseDir, "skills", "portable"), "portable", "Quack skill");
  ...
  it("prefers the provider-native copy and falls back to Quack for that provider", async () => {
    await writeSkill(path.join(synaraBaseDir, "skills", "shared"), "shared", "Quack copy");
    ...
    // A provider without its own copy resolves the Quack fallback.
  ...
  it("uses documented provider alias roots before Quack fallbacks", async () => {
    await writeSkill(path.join(synaraBaseDir, "skills", "shared"), "shared", "Quack copy");
...
`,
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(synaraBaseDir, "skills", "portable"), "portable", "Quack");
