...
> = {
  ...
  feedback: {
    command: "feedback",
    label: "/feedback",
    description: "Send feedback to the Quack team",
    source: "app",
  },
  automation: {
    command: "automation",
  ...
  const availableCommands: ComposerSlashCommand[] =
    input.provider !== "claudeAgent"
      ...
      : [
          ...
          // creates a Quack split/context clone before the provider sees the first turn.
          // /export is app-level too — Quack owns the thread transcript, so the download
