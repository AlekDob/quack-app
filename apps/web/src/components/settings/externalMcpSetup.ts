...
export function buildExternalMcpClientConfiguration(
  ...
  if (client === "codex") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Codex will save Quack as a local MCP server; then open a new Codex task."
        : "Run this command in Terminal. Codex will save Quack as a local MCP server; then open a new Codex task.",
    };
  }
  ...
  if (client === "claudeCode") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Claude Code will make Quack available in all your projects."
        : "Run this command in Terminal. Claude Code will make Quack available in all your projects.",
    };
  }
  ...
  return {
    ...
    instruction:
      client === "claudeDesktop"
        ? "In Claude Desktop, open Settings → Developer → Edit Config. Add the Quack entry without removing existing servers, save, and restart Claude Desktop."
...
export function buildExternalMcpExamplePrompt(projectTitle: string | null): string {
  return [
    projectTitle === null
      ? "Use Quack to create a new task: call synara_overview first, pick the most relevant project, and tell me which one you chose."
      : `Use Quack to create a new task in the project named ${JSON.stringify(projectTitle)}.`,
    "First inspect Quack's capabilities and choose an exact available provider and model; do not guess model names.",
...
// another MCP-capable app). The agent pairs the machine, registers Quack in
...
export function buildExternalMcpSetupPrompt(input: {
  ...
  const sections: string[] = [
    "Connect this coding agent to Quack via MCP. Complete every step yourself, in order, and report what happened.",
  ];
  ...
  sections.push(
    [
      'Step 2 — Register Quack as a stdio MCP server named "synara" in your own configuration, using whichever mechanism your app supports:',
      "",
