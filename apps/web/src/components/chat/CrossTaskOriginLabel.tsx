...
// Purpose: Identify the source thread for conversations created by another Quack agent.
...
// Quack thread, so it always reads as "Sent by Quack" with the Quack mark
// (the origin provider is not surfaced here to keep one consistent label).
function OriginContent(): ReactNode {
  return (
    <>
      <span className="flex size-4 shrink-0 items-center justify-center text-muted-foreground/70">
        <SynaraLogo className="h-4 w-auto" aria-label="Quack" />
      </span>
      <span className="truncate">Sent by Quack from another thread</span>
    </>
  );
}
