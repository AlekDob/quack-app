...
describe("MessagesTimeline", () => {
  ...
  it("labels only the first message when another task created the conversation", async () => {
    ...
    expect(markup).toContain("Sent by Quack from another thread");
    expect(markup).toContain('aria-label="Open source thread"');
    expect(markup.indexOf("Sent by Quack from another thread")).toBeLessThan(
  ...
  it("shows the Quack mark for every provider-specific tool row shape", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    ...
    expect(claudeMarkup).toContain("Quack is creating a thread");
    ...
    expect(codexMarkup).toContain("Quack listed threads");
    ...
    expect(failedMarkup).toContain("Quack couldn&#x27;t create threads");
    expect(failedMarkup).toContain("Claude rejected reasoningEffort");
  });

  // Browser calls get the globe rather than the generic Quack mark: a browsing
  // row is about a page, and the surface it acted on is the first thing to read.
  it("uses the browser icon and action name for Quack browser calls", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...makeTimelineBaseProps()}
    ...
    expect(markup).not.toContain("Quack: Browser Open");
  ...
  it("hides raw `ToolName: {json}` argument details behind the humanized heading", async () => {
    ...
    expect(readThreadMarkup).toContain("Quack read a thread");
    ...
    expect(diagnoseMarkup).toContain("Quack diagnosed a thread");
    ...
    expect(failedArgsMarkup).toContain("Quack couldn&#x27;t create threads");
    expect(failedArgsMarkup).toContain("Invalid params");
  });

  it("keeps Quack tool calls and adds a thread creation recap at the end of the turn", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    ...
    const workEntries = [
      {
        ...
        entry: {
          ...
          itemType: "mcp_tool_call",
          toolName: "mcp__synara__synara_create_threads",
          toolTitle: "Quack created threads",
          activityKind: "tool.completed",
        },
      },
      {
    ...
    expect(liveMarkup).toContain("Quack created threads");
