...
- A `claude:permission-request` arrives → `ClaudePermissionOverlay` runs its gates **in order**: privacy exclusion → read-only allow → `modeAutoAllow(req)` → saved/always-allow rules → show card.
- `modeAutoAllow` calls `getPermModeFor(req)` (session id, then cwd, else `"default"`): `auto` → allow all; `acceptEdits` → allow only `WRITE_TOOLS`; else → no mode-based allow.
