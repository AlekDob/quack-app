...
tags: [codex, multi-backend, agent-abstraction, quack-agent-event, openai-codex, m1, m1.5, backend-only, on-hold]
---

## Codex Backend — Multi-Backend Agent Abstraction (M1)

**Purpose:** Add OpenAI Codex CLI as a second agent backend behind a Rust `AgentBackend` trait + a unified `QuackAgentEvent` translator, with zero behavior change for existing Claude sessions. UI components (session modal, settings menu, model picker) descoped — backend-only abstraction layer.
**Stack:** Rust 1.75+ (Tauri v2, tokio, serde, async-trait), TypeScript strict (adapter + types only)

**Scope:** M1 Foundation (Phases A–E) + event adapter + prompt composer. UI gating, model picker UI, and chat wiring descoped. OAuth `codex login`, MCP/AGENTS.md render, sandbox approval, real child-kill cancel deferred to M2/M3.
**Status:** Backend abstraction complete, UI descoped — feature on hold.
...
| Service | `src-tauri/src/agents/codex_backend.rs` | `CodexBackend` — `codex exec --json` spawn-per-query, emits `codex-event:{agent_id}`; commands `send_message_via_codex` *(in lib.rs)*, `codex_auth_status` |
...
| Model/Type | `src/types.ts` | `AgentSession.backend?`, `backendSessionId?`, `backendAuthMethod?` |
...
| Store/State | `src/stores/sessionStore.ts` | `normalizeSessionBackend()` — legacy sessions default to `'claude'` on load |

### Data Flow
**Codex turn (send):** `invoke('send_message_via_codex')` → `CodexBackend::send_message` → `codex exec --json` subprocess

**Codex events (receive):** codex stdout JSONL → `codex_stream_to_quack` → `QuackAgentEvent` → `emit('codex-event:{agent_id}')` → TS adapter `quackEventToClaudeEvents(ev, seq)` → existing event reducer

**Codex token usage (0.130):** in-stream `turn.completed.usage` → `codex_rollout_to_usage` → `Usage` event via `codex-event`
...
- `gpt-5-codex` — default Codex model (Codex manages its own model; Claude model settings ignored)
...
**Parity behavior:** Codex session (a) writes persona into `AGENTS.md` in the working dir (Codex reads it natively); (b) gets working slash commands the native `codex exec` lacks; (c) gets an injected index of the agent's *selected* skills. Autonomous skill discovery stays Claude-only; **subagents are NOT** (0.130 has them natively — retracted, see re-target section).
**Deferred:** live smoke test; M2 decision inject full SKILL.md body vs index by real token cost.

### M1.5b — Codex model picker (UI REMOVED)
Backend support exists: `build_codex_args` accepts `Option<model>` and passes `-c model=<id>` to Codex CLI. UI components (curated model list, settings panel, model selector) have been descoped. Model override is possible only via direct Rust invocation.
