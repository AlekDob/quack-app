...
- `.ai-composer-shell` wraps `.ai-composer-meta` + `.ai-input-row`; CSS `order` puts textarea on top, controls below, permission/queue on top. Textarea transparent inside the pill. Queue cards: `.ai-queue-stack` / `.ai-queue-card` (feature 039).
