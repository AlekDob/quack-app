...
tags: [ai-chat, compose-card, diff, agent-mode, conductor-style, monaco, undo-keep]
---
...
| Store | `src/store.ts` | `openComposeReview`, `collectComposeReviewTabs`, `focusedComposeReviewKey` |
| Editor shell | `src/components/WorkspaceShell.tsx` | Portals `ComposeReviewPane` into pane containers |
| Agent shell | `src/components/AgentModeShell.tsx` | Reads `crev:` tabs; splits chat \| review |
...
1. Turn includes ≥1 `Write` / `Edit` / `MultiEdit` → **ComposeCard** appears
   (**live during stream**: `N Files · editing…`, list grows; Undo/Keep only after turn ends).
...
| `.agent-main.has-review` | 50/50 row: `.agent-main-chat` \| `.agent-main-review` |
| `.agent-review-tabs` | Multi-file tab strip in agent mode |
