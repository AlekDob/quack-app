...
- **Scroll-to-bottom button placement** — moved from `position: fixed; bottom: 100px; right: 40px` to `position: absolute; top: 12px; right: 16px` with `z-index: 100000`. Rationale: the bottom-right position overlapped with `.edit-summary-bar` (and other bottom overlays) and required a fixed/viewport-anchored position. Absolute + top-right scopes the button to the chat container (`.message-list` / `.message-list-wrapper`, both `position: relative`) and high z-index ensures it floats above any bottom overlay.
