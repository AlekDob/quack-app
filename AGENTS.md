...
Test infra is **vitest** (added 2026-07-17); put pure-logic regression tests
next to the source as `*.test.ts` and run `npm test`. First example:
`src/chatScroll.test.ts` (transcript windowing, feature `080`).
...
| Chat disk persistence | `src-tauri/src/chat_store.rs`, `src/chatStoreCache.ts` — see `043`, lazy hydrate `076` |
...
| Works / tickets / Modules | `src/components/works/`, `src/worksCache.ts`, `src/workItemMd.ts`, `src/quackDir.ts`, `src/surfaceViewPrefs.ts`, `src/editorDrawerStack.ts` — see `054`, `063`, `065` |
