...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `088-plan-milo-handoff.md` — Cursor-style **Pass the ball to Milo** after ExitPlanMode: in-stream CTA (**Open Plan** re-focuses side preview), Features-first, Milo + Agent + auto-send (refs synced before send so stamp/identity stay Milo); Keep discussing / composer send = stay Plan; Agent Mode full-read via right-column Plan tab (`084`).
