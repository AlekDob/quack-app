...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `061-plan-mode-tab.md` — `ExitPlanMode` plan preview: feature drawer when `featureId` linked, else `plan:` split tab; independent of approve/deny.
  ...
  - `088-plan-milo-handoff.md` — Cursor-style **Pass the ball to Milo** after ExitPlanMode: in-stream CTA, Features-first, Milo + Agent + auto-send (refs synced before send so stamp/identity stay Milo); Keep discussing / composer send = stay Plan.
