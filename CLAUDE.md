...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `088-plan-milo-handoff.md` — Cursor-style **Pass the ball to Milo** after ExitPlanMode: in-stream CTA, Features-first, Milo + Agent + auto-send (refs synced before send so stamp/identity stay Milo); Keep discussing / composer send = stay Plan; Agent Mode full-read via right-column Plan tab (`084`).
  ...
  - `084-agent-context-panels.md` — Agent Mode right column: `+` add-view menu, Changes/Files + on-demand Plan (ExitPlanMode) + project Terminal top tabs (`AgentTerminalPanel` / `TerminalCore`); StatusBar panel/Ctrl+J/Ctrl+`/`New Terminal` via `agentContextNav`.
