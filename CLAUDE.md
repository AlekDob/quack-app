...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `084-agent-context-panels.md` — Agent Mode right column: `+` add-view menu, Changes/Files + on-demand Plan (ExitPlanMode) + project Terminal top tabs (`AgentTerminalPanel` / `TerminalCore`); drag-resize width (`agentContextWidth`, default 480); StatusBar panel/Ctrl+J/Ctrl+`/`New Terminal` via `agentContextNav`.
