...
- NEVER boot an iOS simulator with `expo run:ios`, `npm run ios`, or `open -a Simulator`. Those launch Simulator.app, which boots a device Quack did not boot — and Quack deliberately never shuts down a device it does not own (`DeviceManager.releaseDevice`, the `synaraBooted` check). The simulator then survives every Quack restart and burns ~50% CPU forever, unattributed. One was found running for 2 days 19 hours on 2026-08-17. Use the device pane / `device_*` tools, which record ownership in `~/.synara/userdata/device-boot-ownership.json` and reclaim orphans at startup. If a stray one is already up: `xcrun simctl shutdown all`.
