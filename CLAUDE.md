...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `044-provider-session-bridge.md` — unified CLI session bridge (CC/CU/OC): chip, multi-provider ⟲ Sessions picker, `provider_sessions.rs`, thin-row recovery via `chatProviderRecovery.ts`; **no turn-count sid guess**; strip `[Editor context]` on CLI re-import (`chatTextUtils`); **platform pin** locks model picker per chat (`057`).
