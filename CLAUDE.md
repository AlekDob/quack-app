...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `075-chat-switch-loader.md` — gradual translucent veil on chat/session switch (fade + min-floor + cap in `chatSwitch.ts`, `ChatSwitchVeil`); perceived-performance polish. Pairs with the `044` freeze fix. Warm revisits skip the veil (`092`).
  ...
  - `076-chat-lazy-hydrate-done-unload.md` — session index + on-demand bodies; DONE/archived chat hosts unmount when hidden; live stay sticky for multitask; `memo(AIChatHost)` so creating a chat doesn't re-render all mounted panels (`[new-chat-perf]` dev timing). Agent/IDE hub warm set → `092`.
  - `092-agent-chat-warm-lru.md` — last-5 chat MRU + per-ws last pick as `tabOpen` so Agent Mode / hub rail hops stay mounted (CSS toggle, no veil); `memo(AgentChatHost)`; fixes cold remount + cross-ws double hydrate.
  - `093-app-versioning.md` — dual-track SemVer (`develop` → `vX.Y.Z-rc.N`, `production` → `vX.Y.Z`); `bump-version.mjs` + CI draft/prerelease; StatusBar `v…` chip → About.
  ...
  - `015-claude-permission-mode.md` — per-chat permission mode (Ask/Plan/Auto-edit/Auto/Agent); `permModeStore` (Plan never on `byCwd`); overlay auto-allow; ExitPlanMode buy-in → `088`.
  - `061-plan-mode-tab.md` — `ExitPlanMode` plan preview: Agent Mode → right-column Plan tab; IDE → feature drawer when `featureId` linked, else `plan:` split tab; independent of approve/deny.
  - `062-presets.md` — backend-agnostic preset system (Jack + Milo/Nora/Vera/Lia + user-created): model tier/effort/mode/instructions shaping the CURRENT session (not a subagent); Team drawer edits sync to live chat via `subscribePresetSettings`; one merged composer picker (`SubagentPill`: primary agents above, delegable subagents below); create/edit agents with avatars via `AgentCreateDrawer` in the Whiteboard organigramma (`.codetta/presets/`, separate from `.claude/agents/`); custom-preset instructions labeled unverified in the prompt (injection hygiene); **chat identity integrity** via `chatTurnAgent.ts` (per-message `agentId`, mid-turn freeze, session-switch backfill, Pass-the-ball ref sync — bug `003`).
  - `088-plan-milo-handoff.md` — **Pass the ball to Milo** after ExitPlanMode: composer chip only (full plan on right Plan tab / IDE); hidden while AskUserQuestion docked; Features-first; Milo + Agent + auto-send (refs synced before send so stamp/identity stay Milo); Esc / composer send = stay Plan (`084`).
  - `089-chat-auto-title.md` — cheap-LLM chat auto-title after the first turn (CC `--model haiku` / Cursor `-p` / direct-API accumulate); `chatAutoTitle.ts` + `AIChatPanel` trigger; `autoTitled` flag + `preferredTitle()` stop the `deriveTitle` heuristic from clobbering it on flush; hand rename (`titleLocked`) still wins.
  - `016-image-attachments.md` — paste/drag up to 10 images into agentic chats (Claude Code, Cursor CLI); temp disk storage + provider-specific delivery; lost-resume flatten reinjects paths via `wireUserContent` (`cliPrompt.ts`, bug `004`).
  ...
  - `054-pinky-brain-integration.md` — Pinky Brain: hybrid search, opt-in inject gates, `#` composer cites, MCP setup (opt-in via Brain → Setup — not shipped as project `.mcp.json`); `~/.quack/brain` → `~/.pinky/brain` migration.
  - `072-composer-mention-chips.md` — Cursor-style chips for `#` brain / `@` agents; files + features + skills as colored inline text in the textarea (`083`).
  - `083-composer-feature-link.md` — dual cite: inline `@slug` from input; pin chip from icon/auto agent Write of feature `.md`; hub **Feature** badge.
  - `084-agent-context-panels.md` — Agent Mode right column: `+` add-view menu, Changes/Files + on-demand Plan (ExitPlanMode) + project Terminal top tabs (`AgentTerminalPanel` / `TerminalCore`); drag-resize width (`agentContextWidth`, default 480); **collapsed icon rail by default** (`lcp.agent.contextCollapsed`); StatusBar panel/Ctrl+J/Ctrl+`/`New Terminal` via `agentContextNav`.
  - `085-agent-ide-mode-toggle.md` — Agents ↔ IDE layout toggle (`lcp.agentMode`): flush+await before swap; selection survives remount (`agentModeSelection`); cold cost = chat disk hydrate + terminal reattach; warm path keeps rich RAM transcript; `[agent-mode-switch]` timings.
  - `086-perf-audit-window.md` — opt-in companion window: Quack/WebKit process CPU·RAM + switch/new-chat/resume timeline; **Copy JSON** (`quack-perf-audit`); StatusBar Audit chip / Settings Diagnostics / Ctrl+Alt+P; shared StatusBar `process_stats` (no second poll).
  - `090-perf-hardening.md` — 4-phase perf pass: backend commands off the main thread (`spawn_blocking`), lazy Monaco/xterm + isolated `react` chunk (no boot preload), active-first hydrate + 350ms splash, per-token render → ref, `useSyncExternalStore` host status, idle poll/PTY back-off. Deferred: `memo(ChatRow)`, `StreamBuffer.lines` cap.
  ...
  - `022-chat-composer.md` — roomier "spaceship" composer: single-row uniform toolbar, subagent target pill (`SubagentPill`, derived from `attachedAgents`), one effort+thinking popover (`EffortPopover`, slider + segmented), voice dictation (`ComposerMic` + `ComposerDictationBar`, feature 052), `+` attach, hint row, transparent flush textarea + `fitComposerInputHeight` auto-grow; **live turn status dock** (`.ai-status-dock`) above the composer; **per-project context dock** on the right of the same row (`ContextFilesDock`, `workspaceChatContext.ts`); **context usage ring** (CC) + drawer — `023-session-usage-panel.md`.
  - `024-resume-white-screen-recovery.md` — detect resume from macOS standby (`resumeDebug.ts`), heal blank webview (Monaco `layout()` / xterm `fit()`); heal only if hidden ≥**2s** (skip alt-tab flicker); log to console + `localStorage` ring (`__resumeLog()`); also feeds Perf Audit (`086`). Gotcha: Vite compile error blanks the page but never fires `[resume]`.
  ...
  - `026-cursor-cli-bridge.md` — `cursor-agent` spawn/stream/kill; dual parsers (Composer-native + Claude-shaped); images via path-in-prompt; GUI PATH resolve (bug `009`).
  ...
  - `046-process-cleanup.md` — Task Manager (Quack-scoped process tree, Ctrl+Alt+U); PTY process-group kill on terminal close; agent stop on archive/done/close chat (`stopChatAgent`, `aiStopBus`); StatusBar version chip → `093`.
  ...
  - `023-session-usage-panel.md` — composer context ring + Context Usage popover (CC): last-API `contextTokens` snapshot, **`compact_boundary.postTokens` refresh after `/compact`**, Cursor-style segmented breakdown, Usage tab for plan limits + billing; `contextUsage.ts`, `contextBreakdown.ts`, `SessionUsageCircle`, `SessionUsagePopover`.
  ...
  - `031-model-discovery-cache.md` — shared provider/model probe cache (`modelDiscoveryStore`); prefetch at splash; lazy cloud + CLI catalogs; force refresh awaits Cursor probe (`ensureLiveCliCatalogs`); invalidation on API-key edit / force refresh.
  ...
  - `035-macos-release-notarization.md` — local signed/notarized `.dmg` pipeline (`sign-and-notarize.sh`, `.env`, Entitlements); CI macOS still unsigned; version tags → `093`.
  ...
  - `037-project-context-dock.md` — per-workspace "N files in context" pill in the composer status row (right); hover popover lists active editor attach + `@`-queued files; `workspaceChatContext.ts` + `isUnderRoot` guards (absolute sibling paths must not match — bug `006`).
  ...
  - `040-per-session-composer-state.md` — per-session composer draft + CC knobs (effort/mode/thinking) + model; `sessionComposerSeed.ts` + `sessionReady` gate so remounts don't stamp global last-used onto siblings (bug `005`); `composerDraft.ts` (`mergeComposerDraft`, `mergeSessionKnobs` incl. model); Team `applyPreset` only when `chatVisible`.
  - `043-chat-transcript-persistence.md` — disk-backed per-session transcripts (`chat_store.rs`), legacy localStorage migrate, `provider-links.json` reverse index, flush on switch + streaming checkpoint, save-failure toast; concurrent-save race (unique atomic tmp + coalesce); **project-switch never-shrink** (`preferRicherSession`, flush+await before flip); empty new chats seeded in RAM (`087`). Lazy body load: see `076`.
  - `044-provider-session-bridge.md` — unified CLI session bridge (CC/CU/OC): chip, multi-provider ⟲ Sessions picker, `provider_sessions.rs`, thin-row recovery via `chatProviderRecovery.ts`; **no turn-count sid guess**; strip `[Editor context]` on CLI re-import (`chatTextUtils`); **platform pin** locks model picker per chat (`057`); **immediate sid persist + `providerSessionIdsRef`** so project switch cannot fork a new JSONL (bug `004`).
