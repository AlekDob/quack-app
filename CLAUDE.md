...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `025-model-selector.md` — composer chip popover (favorites + groups), full ModelBrowser catalog, ManageModelsModal visibility toggles; prefs in `lcp.modelFavorites` / `lcp.modelDisabled`; **platform pin** for agentic CLIs (`057`).
  ...
  - `057-platform-pin.md` — agentic chats pin to starting CLI; model picker filter + Cursor-style banner/confirm on cross-platform switch; `pinnedProviderId` on `ChatSession`.
