...
| Editor color themes (Monaco syntax, per light/dark) | `src/editorColorThemes.ts`, `src/useResolvedEditorColorTheme.ts`, `src/editorSettings.ts` |
...
- `documentation/features/` — map of the product's durable parts (one doc per component).
  ...
  - `033-editor-color-themes.md` — Monaco syntax themes (VS Code–style picker): catalog filtered by resolved light/dark, separate `lightColorTheme` / `darkColorTheme` in `lcp.editorSettings`, `colorDecorators` for CSS swatches.
